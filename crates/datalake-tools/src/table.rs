//! Table-level Iceberg tools: list, inspect metadata, create, drop, rename.
...
#[tool(
    ...
    description = "List Iceberg tables within a namespace. Returns table names (not their full identifiers)."
)]
...
impl ToolFunction for IcebergListTablesTool {
    ...
    async fn run(&self, input: Self::Input) -> Result<ToolResult, ToolError> {
        let catalog = Datalake::default().get_catalog().await.map_err(err)?;
...
impl ToolFunction for IcebergCreateTableTool {
    ...
    async fn run(&self, input: Self::Input) -> Result<ToolResult, ToolError> {
        ...
        let catalog = datalake.get_catalog().await.map_err(err)?;
        let namespace = crate::common::parse_namespace(&input.namespace).map_err(err)?;
        let ident = table_ident(&input.namespace, &input.table).map_err(err)?;
